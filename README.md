...
# 十一、 函数式模型
## 11.1 序贯式模型的局限性及函数式模型的特点
    序贯式模型可以看作是函数式模型的一个特例
![img_1.png](imgs/img_19.png)
### 11.1 例子
    比如这样的建模任务:“根据输人的文字内容，建立一个模型，预测文字的风格类型和写作年代。”该任务的输出有两个:风格类型、写作年代。这种任务就无法用一个序贯模型来解决，可以使用更加灵活的函数式模型
...
## 11.4 多输出模型的编译技巧
    1.多输出模型中，不同输出的性质可能不同，比如年龄输出是回归任务，而性别输出是分类任务，这种情况下需要为不同输出指定不同的损失的数。模型会计算各自的损失值得到一个全局损失，并在训练过程中通过反向传播算法将该损失值最小化。
    2.另外还可以为每个损失值对全局损失的责献分配不同大小的权重(即自定义每一路输出的损失的重要性)。当不同的损失值具有不同的取值范围(量纲)时，这一方法其有用，具体的权重值需要针对实际任务和数据通过调参来确定
## 11.5 为不同输出指定不同的损失函数
    不同输出的性质可能不同，比如年龄输出是回归任务，而性别输出是分类任务，这种情况下需要为不同输出指定不同的损失函数。模型会计算各自的损失值得到一个全局损失，并在训练过程中通过反向传播算法将该损失值最小化。
## 11.6 为不同输出指定不同的权重
    还可以为每个损失值对全局损失的贡献分配不同大小的权重(即自定义每一路输出的损失的重要性)。当不同的损失值具有不同的取值范围(量纲)时，这一方法尤其有用。具体的权重需要针对实际任务和数据调参来确定。
